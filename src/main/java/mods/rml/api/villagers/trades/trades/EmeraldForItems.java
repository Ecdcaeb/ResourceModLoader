...
public class EmeraldForItems {
    public static class Loader extends TradeBase.Loader{
        ...
        public EntityVillager.ITradeList loadTrade(JsonObject trade) {
            ...
            if(to.has("item")){
                return new mods.rml.api.villagers.trades.itrades.EmeraldForItems(TradeBase.getFullItemStack(JsonUtils.getJsonObject(to,"item")),
                        TradeBase.loadPrice(JsonUtils.getJsonObject(from,"price")
                        ));
            }
            else return new mods.rml.api.villagers.trades.itrades.EmeraldForItems(TradeBase.getFullItemStack(JsonUtils.getJsonObject(from,"item")),
